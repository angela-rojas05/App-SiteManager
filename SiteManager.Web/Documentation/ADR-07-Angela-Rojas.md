# ADR-07: SiteManager — Suite de pruebas + pipeline CI
| Campo  | Valor |
|--------|-------|
| Autor  | Ángela Rojas |
| Fecha  | 22/07/2026 |
...
Como parte del pipeline de integración continua se implementó una suite de pruebas con xUnit. Las clases elegidas y su justificación son:

| Clase probada | Tipo | Por qué se eligió |
|---|---|---|
| `ClienteService` | Application | Es el servicio más representativo del patrón Repository. Verifica que agregar, obtener, actualizar y eliminar clientes funciona correctamente a través de la interfaz. |
| `SiniestroService` | Application | Es la clase que implementa el patrón Observer. Se eligió para verificar que al actualizar un siniestro, los observers son notificados correctamente. |
| `JsonSiniestroRepository` | Infrastructure | Es la implementación concreta del repositorio que trabaja con archivos JSON. Se eligió para probar la capa de Infrastructure con archivos temporales reales, cubriendo la deuda técnica identificada en este mismo ADR. |

Las pruebas siguen el patrón **Arrange-Act-Assert** y se ejecutan automáticamente en cada push mediante un workflow de GitHub Actions configurado en `.github/workflows/ci.yml`.
